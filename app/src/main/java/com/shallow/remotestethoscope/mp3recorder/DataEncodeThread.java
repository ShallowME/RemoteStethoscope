...
public class DataEncodeThread extends HandlerThread implements AudioRecord.OnRecordPositionUpdateListener {
    ...
    /**
     *
     * Read data and analyse it from buffer
     * Encode data into MP3 using Lame.
     *
     * @return The length of data from buffer
     * return 0 when no data in buffer
     */
    private int processData() {
        if (mTasks.size() > 0) {
            ...
            int encodeSize = LameUtil.encode(buffer, buffer, readSize, mMp3Buffer);
            if (encodeSize > 0) {
                try {
